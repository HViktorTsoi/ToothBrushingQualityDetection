...
public class SpectralSubtraction {
    ...
    private final static double betaMin = 0.002;
    private double noiseSignalLength;
    ...
    public double noiseAverage(double[] recordNoise) {
        ...
        noiseEnergy = 0;
        // 动态设定前置噪声的长度
        int totalFrames = recordNoise.length / frameSize;
        double dynamicNoiseSignalLength = totalFrames < this.noiseSignalLength
                ? totalFrames / 3
                : this.noiseSignalLength;
        System.out.println("totalFrames:" + totalFrames);
        System.out.println("dynamicNoiseSignalLength:" + dynamicNoiseSignalLength);
        for (int i = 0; i < dynamicNoiseSignalLength; i++) {
            start = i * frameSize / 4;
            end = start + frameSize;
    ...
    public short[] noiseSubtraction() {
        ...
        for (int i = 0; i < numFrames; i++) {
            ...
            for (int b = 0; b < frameSize; b++) {
                if (specY[b] <= 0) {
                    specX[b] = 0;
                } else if (specY[b] > (alpha + beta) * specNoise[b]) {
